...
class ProgramacionController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = Programacion::with(['zone', 'schedule', 'vehicle', 'conductor', 'ayudante1', 'ayudante2', 'group'])
            ...
            return DataTables::of($query)
                ->addColumn('fecha_fmt', fn($p) => Carbon::parse($p->date)->format('d/m/Y'))
                ...
                ->addColumn('zona_name', fn($p) => $p->zone->name ?? '-')
                ->addColumn('turno_name', function ($p) {
                    if (!$p->schedule) return '-';
                    return '<span class="badge badge-light border px-2 py-1">' . $p->schedule->name . '</span>';
                })
                ->addColumn('vehicle_name', function ($p) {
                    if (!$p->vehicle) return '-';
                    return '<strong>' . $p->vehicle->name . '</strong><br><small class="text-muted">' . $p->vehicle->code . '</small>';
                })
                ->addColumn('conductor_name', fn($p) => $p->conductor->name ?? '-')
                ->addColumn('ayudantes_names', function ($p) {
                    $names = '<small>' . ($p->ayudante1->name ?? '-') . '</small>';
                    if ($p->ayudante2) {
                        $names .= '<br><small class="text-muted">' . $p->ayudante2->name . '</small>';
                    }
                    return $names;
                })
                ->addColumn('actions', function ($p) {
                    $btnView = '<button class="btn btn-sm btn-info btn-ver mr-1" data-id="' . $p->id . '" title="Ver Detalle"><i class="fas fa-eye text-white"></i> Ver Detalle</button>';
                    $btnEdit = '<button class="btn btn-sm btn-warning btn-editar mr-1" data-id="' . $p->id . '" title="Editar"><i class="fas fa-pen text-dark"></i></button>';
                    $btnFinish = '';
                    if ($p->status === 'Programado') {
                        $btnFinish = '<button class="btn btn-sm btn-success btn-finalizar mr-1" data-id="' . $p->id . '" title="Finalizar"><i class="fas fa-flag-checkered text-white"></i></button>';
                    }
                    $btnDelete = '<form action="' . route('admin.programacion.destroy', $p->id) . '" method="POST" class="frmEliminar d-inline" style="margin:0;padding:0;">'
    ...
    public function store(Request $request)
    {
        try {
            ...
            while ($current->lte($end)) {
                $dayOfWeek = $current->dayOfWeek; // 0=Sunday, 1=Mon ... 6=Sat
                if (in_array($dayOfWeek, $workDays)) {
                    $exists = Programacion::where('date', $current->toDateString())
            ...
            DB::commit();
            return response()->json(['message' => "Programación guardada correctamente. Se generaron {$created} registro(s)."], 200);
        } catch (\Throwable $th) {
            DB::rollBack();
    ...
    {
        $prog = Programacion::with(['zone', 'schedule', 'vehicle', 'conductor', 'ayudante1', 'ayudante2', 'group'])->findOrFail($id);
        return response()->json($prog);
    }

    public function edit($id)
    {
        $prog      = Programacion::with(['zone', 'schedule', 'vehicle', 'conductor', 'ayudante1', 'ayudante2'])->findOrFail($id);
        $groups    = PersonalGroup::where('status', 'Activo')->orderBy('name')->get();
        $users     = User::whereHas('contracts', fn($q) => $q->where('active', true))->orderBy('name')->get();
        return view('admin.programaciones.template.form_edit', compact('prog', 'groups', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $prog->update($request->only(['conductor_id', 'ayudante1_id', 'ayudante2_id', 'status', 'observations']));
    ...
    {
        try {
            Programacion::findOrFail($id)->delete();
    ...
    public function finalize($id)
    {
        try {
            ...
            if ($prog->status !== 'Programado') {
                return response()->json(['message' => 'La programación ya fue finalizada.'], 422);
            }
    ...
    public function validateAvailability(Request $request)
    {
        ...
        foreach ($peopleIds as $role => $userId) {
            ...
            // Check active contract covers the period
