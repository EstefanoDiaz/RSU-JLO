...
{!! Form::open([
    'route'  => ['admin.programacion.update', $prog->id],
    'method' => 'PUT',
    'id'     => 'formEditProgramacion',
]) !!}

<div class="row mb-3">
    <div class="col-md-6">
        <small class="text-muted text-uppercase font-weight-bold">Fecha</small>
        <p class="font-weight-bold mb-0">{{ \Carbon\Carbon::parse($prog->date)->format('d/m/Y') }}</p>
    </div>
    <div class="col-md-6">
        <small class="text-muted text-uppercase font-weight-bold">Grupo</small>
        <p class="font-weight-bold mb-0">{{ $prog->group->name ?? '-' }}</p>
    </div>
</div>
    ...
    <div class="col-md-4 form-group mb-3">
        ...
        <select name="conductor_id" class="form-control" required>
            @foreach($users as $user)
                <option value="{{ $user->id }}" {{ $prog->conductor_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-4 form-group mb-3">
        ...
        <select name="ayudante1_id" class="form-control" required>
            @foreach($users as $user)
                <option value="{{ $user->id }}" {{ $prog->ayudante1_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-4 form-group mb-3">
        ...
        <select name="ayudante2_id" class="form-control">
            ...
            @foreach($users as $user)
                <option value="{{ $user->id }}" {{ $prog->ayudante2_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
            @endforeach
        </select>
    </div>
</div>
...
<div class="form-group mb-3">
    ...
    {!! Form::select('status', ['Programado' => 'Programado', 'Finalizado' => 'Finalizado'], $prog->status, ['class' => 'form-control', 'required' => true]) !!}
</div>

<div class="form-group mb-3">
    {!! Form::label('observations', 'Observaciones', ['class' => 'font-weight-bold text-xs text-secondary text-uppercase']) !!}
    {!! Form::textarea('observations', $prog->observations, ['class' => 'form-control', 'rows' => 2, 'placeholder' => 'Observaciones adicionales...']) !!}
</div>
